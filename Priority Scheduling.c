...
void main()
{
    int burstTime[10],p[10],waitingtTime[10],turnAroundTime[10],priority[10];
    int i,j,temp,numOfProcess,total=0,k,averageWaitingTime;
    do{
  ...
  {
      printf("\nEnter Burst Time for per Processor P(%d): ",i+1);
      scanf("%d",&burstTime[i]);
      printf("Enter Priority   for per Processor P(%d): ",i+1);
      scanf("%d",&priority[i]);
      p[i]=i+1;
  }
    ...
    averageWaitingTime = total/numOfProcess;
    total=0;

    printf("\nProcess Number  Burst Time      Priority         Waiting Time  TurnAround Time\n");

    for(i=0;i<numOfProcess;i++)
    {
        turnAroundTime[i] = burstTime[i] + waitingtTime[i];
        total+=turnAroundTime[i];
        printf("\nP[%d]\t\t%d\t\t%d\t\t  %d   \t\t%d",p[i],burstTime[i],priority[i],waitingtTime[i],turnAroundTime[i]);
    }
